...
# For F.r.i.d.a.y And IndianAndencento ™
""" UserAndencento module which contains everything related to \
...
from userAndencento import LOGS, CMD_HELP, TEMP_DOWNLOAD_DIRECTORY
...
async def download(target_file):
    """ For .dl command, download files to the userAndencento's server. """
    await target_file.edit("Processing using userAndencento server ( ◜‿◝ )♡")
...
async def uploadir(udir_event):
    ...
    if os.path.exists(input_str):
        await udir_event.edit("Downloading Using UserAndencento Server....")
        lst_of_files = []
...
async def upload(u_event):
    """ For .upload command, allows you to upload a file from the userAndencento's server """
    await u_event.edit("Processing ...")
    input_str = u_event.pattern_match.group(1)
    if input_str in ("userAndencento.session", "config.env"):
