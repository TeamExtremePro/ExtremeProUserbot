...
@bot.on(amanpandey_cmd(pattern="addblacklist ((.|\n)*)", allow_sudo=True))
...
@bot.on(amanpandey_cmd(pattern="rmblacklist ((.|\n)*)", allow_sudo=True))
...
@bot.on(amanpandey_cmd(pattern="listblacklist$", allow_sudo=True))
...
CMD_HELP.update(
    ...
    \n\n**Note : **if you are adding more than one word at time via this, then remember that new word must be given in a new line that is not [hi ExtremeProo]. It must be as\
    \n[hi \n ExtremeProo]"
    }
)
