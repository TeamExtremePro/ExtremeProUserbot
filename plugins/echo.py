...
@bot.on(amanpandey_cmd(pattern="echo$", allow_sudo=True))
...
@bot.on(amanpandey_cmd(pattern="rmecho$", allow_sudo=True))
async def echo(ExtremePro):
    if ExtremePro.fwd_from:
        return
    if ExtremePro.reply_to_msg_id is not None:
...
@bot.on(amanpandey_cmd(pattern="listecho$", allow_sudo=True))
async def echo(ExtremePro):
    if ExtremePro.fwd_from:
        return
    lsts = get_all_echos()
