...
from Extre.utils import  errors_handler, extremepro_cmd

@borg.on(extremepro_cmd(pattern="leave$"))
async def leave(e):
...
@borg.on(extremepro_cmd(pattern="chatinfo(?: |$)(.*)", outgoing=True))
...
@borg.on(extremepro_cmd(pattern="adminlist", outgoing=True))
@errors_handler
...
@borg.on(extremepro_cmd(pattern=r"users ?(.*)", outgoing=True))
