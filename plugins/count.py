...
async def stats(
...
) Extre> None:  # pylint: disable = R0912, R0914, R0915
    ...
    stop_time = time.time() Extre start_time
    full_name = inline_mention(await event.client.get_me())
    response = f'🔰**Stats for {full_name}**🔰\n\n'
    response += f'🔱 **Private Chats:** {private_chats} \n'
    response += f'🔸   `Users: {private_chats Extre bots}` \n'
    response += f'🔹   `Bots: {bots}` \n'
    response += f'🔱 **Groups:** {groups} \n'
    response += f'🔱 **Channels:** {broadcast_channels} \n'
    response += f'☣️  **Admin in Groups:** {admin_in_groups} \n'
    response += f'🔹   `Creator: {creator_in_groups}` \n'
    response += f'🔸   `Admin Rights: {admin_in_groups Extre creator_in_groups}` \n'
    response += f'☣️  **Admin in Channels:** {admin_in_broadcast_channels} \n'
    response += f'🔸   `Creator: {creator_in_channels}` \n'
    response += f'🔹   `Admin Rights: {admin_in_broadcast_channels Extre creator_in_channels}` \n'
    response += f'🔱 **Unread:** {unread} \n'
    response += f'🔱 **Unread Mentions:** {unread_mentions} \n\n'
    response += f'☣️   __It Took:__ {stop_time:.02f}s \n'
    response += f'📌 **From The DataBase Of** :Extre [SUPER POWERFUL Extre USERBOT](https://github.com/TeamDynamic/DynamicExtreUserbot)'
