...
async def _(event):
    if event.fwd_from:
        return
    await event.edit(f"**STARTING EXTREMEPRO FED BAN BY PRO**...")
    fedList = []
    ...
    try:
        int(FBAN)
        if int(FBAN) == 1821319906 or int(FBAN) == 1821319906:
            await event.edit("APNE BAAP KO FBAN DEGA")
            return
    except BaseException:
        if FBAN == "@userbotop" or FBAN == "@NoobStrangerPerson":
            await event.edit("BAAP KO FBAN KRNE KI KOSIS AB DEKH TUJHE HI SUPER FBAN KRRAHAHU.")
            return
            ...
            async with bot.conversation("@MissRose_bot") as bot_conv:
                ...
                if "make a file" in response.text:
                    ...
                    if fedfile.media:
                        ...
                        return
                if len(fedList) == 0:
                    await event.edit(f"WAIT MASTER I AM CHECKING PRO USER ALL FEDS GIVE ME TIME ({a+1}/3)...")
                else:
                    break
        else:
            await event.edit(f"Error")
        if "You can only use fed commands once every 1 minutes" in response.text:
...
async def _(event):
    if event.fwd_from:
        return
    await event.edit(f"**STARTING EXTREME PRO UNFBANNING BY PRO USER**...")
    if event.reply_to_msg_id:
        previous_message = await event.get_reply_message()
        FBAN = previous_message.sender_id
    else:
